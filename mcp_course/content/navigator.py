...
from mcp_course.models.content import ContentLoader
...
class CourseNavigator:
        ...
        try:
            ...
            if current_index + 1 < len(context.available_modules):
                ...
                if next_module.lessons:
                    first_lesson = min(next_module.lessons, key=lambda lesson: lesson.order)
                    return NavigationResult(
                        success=True,
        ...
        try:
            ...
            if current_index > 0:
                ...
                if prev_module.lessons:
                    last_lesson = max(prev_module.lessons, key=lambda lesson: lesson.order)
                    return NavigationResult(
                        success=True,
    ...
    def get_learning_path(self, context: NavigationContext) -> list[tuple[str, str]]:
        ...
        for module_id in context.available_modules:
            ...
            if module:
                for lesson in sorted(module.lessons, key=lambda lesson: lesson.order):
